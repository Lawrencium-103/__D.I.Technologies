...
// Resolve the module directory WITHOUT redeclaring `__dirname` (Netlify's
// esbuild bundler already injects it, which caused a "duplicate declaration"
// SyntaxError and broke every function route). Prefer the injected/global
// value, then import.meta.dirname (Node 20.11+), then fall back — all wrapped
// so a missing/odd import.meta can never throw at module load.
let __omdir = process.cwd()
try {
  if (globalThis.__dirname) {
    __omdir = globalThis.__dirname
  } else if (typeof import.meta.dirname === 'string') {
    __omdir = import.meta.dirname
  } else if (import.meta && typeof import.meta.url === 'string') {
    __omdir = path.dirname(fileURLToPath(import.meta.url))
  }
} catch {
  __omdir = process.cwd()
}
...
export function loadEnv() {
  try {
    const txt = fs.readFileSync(path.join(__omdir, '..', '.env'), 'utf8')
...
export function buildProviders() {
  const list = []
  // Accept both our DIT_* names and the conventional provider names people
  // actually use when pasting keys into a host's dashboard.
  const groqKey = process.env.DIT_LLM_API_KEY || process.env.GROQ_API_KEY || process.env.GROQ_API_TOKEN
  if (groqKey) {
    list.push({
      name: 'groq',
      base: process.env.DIT_LLM_BASE_URL || 'https://api.groq.com/openai/v1',
      key: groqKey,
      model: process.env.DIT_LLM_MODEL || process.env.GROQ_MODEL || 'llama-3.3-70b-versatile',
    })
  }
  const nvKey =
    process.env.DIT_NV1 || process.env.NVIDIA_API_KEY || process.env.NVAPI_KEY || process.env.NVIDIA_API_TOKEN
  const nv = [
    { k: process.env.DIT_NV1 || nvKey, m: process.env.DIT_NV1_MODEL || 'nvidia/nemotron-3-super-120b-a12b' },
    { k: process.env.DIT_NV2 || nvKey, m: process.env.DIT_NV2_MODEL || 'meta/llama-3.2-1b-instruct' },
    { k: process.env.DIT_NV3 || nvKey, m: process.env.DIT_NV3_MODEL || 'openai/gpt-oss-120b' },
  ]
  for (const x of nv) {
...
export async function webSearch(query) {
  const key = process.env.DIT_SEARCH_API_KEY || process.env.TAVILY_API_KEY || process.env.TAVILY_API_TOKEN
  if (key) {
    try {
